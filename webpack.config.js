//使用了CommonJS规范化的一个电商项目前端的webpack配置
//使用了CommonsChunkPlugin组件，将公共部分提取出来，放在了dist/js/base.js中
//html、css、js 分别进行打包
...
var config = {
	...
	output: { //打包出口 目标文件，__dirname:运行时的当前路径
        ...
        publicPath  : 'dev' === WEBPACK_ENV ? '/dist/' : '//s.happymmall.com/qeb-fe/dist/',//上线使用，访问文件路径
        filename    : 'js/[name].js'
    },
	externals :{//外部依赖声明
	...
	resolve : {         //配置目录别名
        alias : {
            ...
            util            : __dirname + '/src/util',     //存在项目中，业务逻辑代码的工具代码
            page            : __dirname + '/src/page',    //存放css、js 
            service         : __dirname + '/src/service', //存放与后端进行交互的代码
            image           : __dirname + '/src/image'   //图片
        }
    },
	plugins:[//插件
		//独立 通用模块
    ...
    new HtmlWebpackPlugin(getHtmlConfig('payment', '订单支付')),
		...
		new HtmlWebpackPlugin(getHtmlConfig('about', '关于QeB'))
	]
};
